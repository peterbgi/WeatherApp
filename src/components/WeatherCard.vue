...
<template>
  <div
    ...
    class="text-white p-10 rounded-lg bg-blue-600 shadow-lg gap-6 mb-6 relative overflow-hidden"
  >
    ...
    <div class="text-center flex-1">
      ...
      <h1 class="text-9xl mb-2">{{ Math.round(place.current.temp_c) }}<sup>°</sup></h1>
    ...
    <Transition name="fade">
      <div v-show="showDetails">
        <WeatherInfo
          ...
          @delete-place="removePlace(place.location.name)"
        />
      </div>
    </Transition>
    ...
    <div class="flex justify-end items-center gap-1 mt-10">
      <button @click="showDetails = true">
        Több adat <i class="fa-solid fa-arrow-right text-sm -mb-px"></i>
      </button>
    </div>
  </div>
</template>
